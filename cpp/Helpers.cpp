...
static std::vector<float> loadImage(const std::string& filename, int sizeX = 224, int sizeY = 224)
{
    ...
    // Normalize the channel wrt ImageNet values
    std::vector<cv::Mat> channels;
    cv::split(image, channels);
    channels[0] = (channels[0]/255 - 0.485) / 0.229;
    channels[1] = (channels[1]/255 - 0.456) / 0.224;
    channels[2] = (channels[2]/255 - 0.406) / 0.225;
